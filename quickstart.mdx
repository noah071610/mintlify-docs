---
title: "연동 시작하기"
description: "카나페 이벤트와 제품을 빠르게 연동해보세요"
---

## Step 1: 카나페 이벤트 iframe과 SDK 삽입

SDK를 카나페 이벤트 iframe과 같이 삽입해 주세요
...
<AccordionGroup>
  <Accordion icon="rectangle-terminal" title="최신 버전 삽입">
    ...
    <script
      async
      src="https://cdn.canape.im/_static/js/canape-embed-helper/@latest/canape-embed-helper.js?event_id=[EVENT_HASH]"
    ></script>
    ```
  </Accordion>
...
if (window.CanapeEmbedHelper) {
  CanapeEmbedHelper.init([TARGET_ORIGIN], [INITIAL_DATA?]);
}
```

## Step 3-1: SDK를 사용한 초기화값 설정

SDK를 초기화하고 2번째 인자(initial data)에 초기화 값 오브젝트를 넣어주세요
...
<Info>보안을 위해 2번째 인자의 origin을 꼭 확인해주세요</Info>
