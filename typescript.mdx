...
<AccordionGroup>
  ...
  <Accordion icon="rectangle-terminal" title="브라우저에서 직접 다운로드">
    1. 브라우저에서 `http://cdn.canape.im/@latest/CanapeEmbedHelper.d.ts` 주소로 이동
    2. 페이지 내용을 복사하거나 "다른 이름으로 저장" 기능 사용
    3. 파일명을 `CanapeEmbedHelper.d.ts`로 저장
  </Accordion>
</AccordionGroup>
  ...
  interface Window {
    CanapeEmbedHelper: {
      ...
      event: {
        ...
        send(type: "initializeData", data: InitializeData): void
